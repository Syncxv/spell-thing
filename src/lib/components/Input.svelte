...
<div class={`relative h-10 w-full min-w-[200px] ${wrapperClassName}`}>
	{#if type === 'text'}
		<input {id} type="text" placeholder=" " class={className} bind:value on:input={handleChange} />
	{:else if type === 'number'}
		<input {id} type="number" {min} {max} class={className} bind:value on:input={handleChange} />
	{/if}
	<label
		for={id}
