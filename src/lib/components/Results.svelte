<script lang="ts">
	...
	import hehe, { Wrapper } from '$lib/spell-cast-word-finder/pkg/scf';
	...
	async function handleSubmit() {
		...
		if (wordLen <= 8) {
			...
			const solver = new Wrapper(Array.from(validWordsSet), combo);
			const tempRes: Letter[][][] = [];
			for (let i = 1; i < wordLen + 1; ++i) {
				tempRes.push(solver.get_combos(i));
			}

			results = tempRes.map((e) => e.map((t) => t.map((l) => $letterMatrixStore[l.row][l.col])));
		}
